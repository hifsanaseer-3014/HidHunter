...
def upload_to_virustotal(path):
    ...
    for attempt in range(10):
        ...
        if r.status_code == 200:
            ...
            logging.info(f"VT upload-scan poll {attempt + 1}/10: status={status}")
            if status == "completed":
...
def scan_zip(path):
    """Extract suspicious files from a ZIP into a temp dir and return their
    real paths, plus a failure reason string if the archive couldn't be read
    at all (corrupt or password-protected) — this should be surfaced to the
    user rather than silently ignored, since a locked archive is itself
    sometimes used as a malware evasion tactic."""
    suspicious_paths = []
    fail_reason = None
    try:
                ...
                if ext in SUSPICIOUS_EXT:
                    try:
                        ...
                        if "password" in str(e).lower() or "encrypted" in str(e).lower():
                            fail_reason = "password-protected — could not scan contents"
                        else:
                            logging.error(f"ZIP extract error {name}: {e}")
                    except Exception as e:
                        logging.error(f"ZIP extract error {name}: {e}")
    except zipfile.BadZipFile:
        fail_reason = "corrupt or invalid ZIP — could not scan contents"
        logging.error(f"ZIP scan error {path}: corrupt/invalid archive")
    except Exception as e:
        logging.error(f"ZIP scan error {path}: {e}")
    return suspicious_paths, fail_reason


def scan_rar(path):
    """Extract suspicious files from a RAR into a temp dir and return their
    real paths, plus a failure reason string if the archive couldn't be read
    ...
    try:
        ...
        with rarfile.RarFile(path, 'r') as r:
            if r.needs_password():
                return [], "password-protected — could not scan contents"
        ...
        fail_reason = "corrupt or invalid RAR — could not scan contents"
        logging.error(f"RAR scan error {path}: corrupt/invalid archive")
    except Exception as e:
...
def add_to_startup():
    """Register HIDHunter to launch at login via a single, standard,
    user-visible mechanism (the registry Run key) — the same mechanism
    countless legitimate apps use. Deliberately no self-reinstalling
    watchdog/scheduled-task backup: a tool that can be cleanly stopped
    with one action (Task Manager, or the tray Quit button) is more
    trustworthy than one that silently resurrects itself, both to the
    user and to AV/EDR heuristics."""
    try:
        key = winreg.OpenKey(
...
class DropHandler(FileSystemEventHandler):
    def _check(self, event):
        ...
        if ext in (".zip", ".rar"):
            extracted_items, fail_reason = scan_zip(path) if ext == ".zip" else scan_rar(path)

            if fail_reason:
                logging.warning(f"ARCHIVE UNREADABLE: {path.name} — {fail_reason}")
                with alert_lock:
                    alert_count += 1
                log_alert(path, "N/A", extra=f"| ⚠ Could not scan: {fail_reason}")
                notification.notify(
                    title="HIDHunter: ⚠ Couldn't Scan Archive",
                    message=f"{path.name}\n{fail_reason}\nTreat this file with caution.",
                    timeout=10
                )
                play_alert_sound(is_dangerous=False)
